"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.MetaadsController = void 0;
const common_1 = require("@nestjs/common");
const metaads_service_1 = require("./metaads.service");
let MetaadsController = class MetaadsController {
    metaadsService;
    constructor(metaadsService) {
        this.metaadsService = metaadsService;
    }
    async getAdAccounts(accessToken) {
        return this.metaadsService.getAdAccounts(accessToken);
    }
    async getInsights(adAccountId, datepreset, accessToken) {
        return this.metaadsService.getInsights(adAccountId, datepreset, accessToken);
    }
};
exports.MetaadsController = MetaadsController;
__decorate([
    (0, common_1.Get)('adaccounts'),
    __param(0, (0, common_1.Query)('accessToken')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", Promise)
], MetaadsController.prototype, "getAdAccounts", null);
__decorate([
    (0, common_1.Get)('insights'),
    __param(0, (0, common_1.Query)('adAccountId')),
    __param(1, (0, common_1.Query)('datepreset')),
    __param(2, (0, common_1.Query)('accessToken')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, String, String]),
    __metadata("design:returntype", Promise)
], MetaadsController.prototype, "getInsights", null);
exports.MetaadsController = MetaadsController = __decorate([
    (0, common_1.Controller)('metaads'),
    __metadata("design:paramtypes", [metaads_service_1.MetaadsService])
], MetaadsController);
//# sourceMappingURL=metaads.controller.js.map